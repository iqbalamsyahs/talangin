...
export const groups = pgTable("groups", {
    ...
    createdBy: text("created_by").notNull().references(() => users.id), // ADMIN GRUP
    createdAt: timestamp("created_at").defaultNow(),
});
...
export const expenses = pgTable("expenses", {
    ...
    payerMemberId: uuid("payer_member_id").notNull().references(() => groupMembers.id),
...
export const expenseItems = pgTable("expense_items", {
    ...
    assignedToMemberId: uuid("assigned_to_member_id").notNull().references(() => groupMembers.id),
});
...
export const expenseSplits = pgTable("expense_splits", {
    ...
    memberId: uuid("member_id").notNull().references(() => groupMembers.id), // Siapa yang berhutang
...
export const expensesRelations = relations(expenses, ({ one, many }) => ({
    ...
    payer: one(groupMembers, { fields: [expenses.payerMemberId], references: [groupMembers.id] }),
...
export const expenseItemsRelations = relations(expenseItems, ({ one }) => ({
    ...
    assignedMember: one(groupMembers, { fields: [expenseItems.assignedToMemberId], references: [groupMembers.id] }),
}));
